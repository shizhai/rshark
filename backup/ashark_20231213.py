    ...
    def __init__(self, root):
        ...
        if self.parse_on_time:
            ...
            for ptitle in self.ptitle_fields:
                ...
                ptitles["t_value"+ptitle] = tk.Entry(self.data_frame, textvariable=ptitles["t"+ptitle], state="readonly", justify="center", width=8)
            ...
            for idx in range(0, len(self.psub_fields) + 1, 1):
                for psub in self.psub_fields:
                    ...
                    psubs["t_value"+psub+str(idx)] = tk.Entry(self.data_frame, textvariable=psubs["t"+psub+str(idx)], state="readonly", justify="center", width=8)
...
class ScrollableFrame(tk.Frame):
    def __init__(self, master, maxheight, maxwidth, *args, **kwargs):
        ...
        # 创建 Canvas，设置垂直滚动条
        self.canvas = tk.Canvas(self, yscrollcommand=vscrollbar.set, width=maxwidth, height=maxheight, yscrollincrement=100)
    ...
    def _on_mousewheel(self, event):
        # 响应滚轮事件
        if event.delta:
            print(event.delta, "cur:" + str(self.canvas.canvasy(0)))
            if event.delta < 0:
                if self.canvas.canvasy(0) - event.delta <= 800:
                    self.canvas.yview_scroll(int(-1 * (event.delta / 100)), 'units')

            if event.delta > 0:
                if self.canvas.canvasy(0) - event.delta > 0:
                    self.canvas.yview_scroll(int(-1 * (event.delta / 100)), 'units')
...
if __name__ == "__main__":
    ...
    # print(gv)
    # root.geometry(gv)
    root.maxsize(width=1600, height=800) 
    # app = NetworkTestGUI(root)
    # root.mainloop()

    # root.update_idletasks()

    scrollable_frame = ScrollableFrame(root, 800, 1600)
    scrollable_frame.pack(fill='both', expand=True)
    app = NetworkTestGUI(scrollable_frame.interior)

    # root.update_idletasks()
    # root.update()
    # gv = str(root.winfo_width()) + "x" + str(root.winfo_height())
    # root.geometry(gv)
