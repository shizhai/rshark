...
class ScrollableFrame(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        # 创建垂直滚动条
        vscrollbar = tk.Scrollbar(self, orient='vertical', troughcolor='gray', width=12)
        vscrollbar.pack(side='right', fill='y', expand=False)

        # 创建 Canvas，设置垂直滚动条
        self.canvas = tk.Canvas(self, yscrollcommand=vscrollbar.set)
    ...
    def _on_drag(self, event):
        ...
        self.start_y = y
        self.canvas.yview_scroll(int(-1 * (delta_y / 2)), 'units')
...
class NetworkTestGUI:
    def __init__(self, root):
        ...
        self.title_sub_start_column = 4

        ptitles = {}
        ptitles["tmenu"] = tk.StringVar()
        ptitles["tmenu"].set("src-->dst")
        ptitles["t_value_menu"] = tk.Entry(self.root, textvariable=ptitles["tmenu"], state="readonly", justify="center")
        ptitles["t_value_menu"].grid(row=0, column=3, rowspan=2, padx=1, pady=0, sticky="we")
        ...
        if self.parse_on_time:
            ...
            for ptitle in self.ptitle_fields:
                ...
                ptitles["t_value"+ptitle] = tk.Entry(self.root, textvariable=ptitles["t"+ptitle], state="readonly", justify="center", width=8)
                ptitles["t_value"+ptitle].grid(row=0, column=self.title_sub_start_column + self.ptitle_fields[ptitle] * len(self.psub_fields),
                                               columnspan=len(self.psub_fields), padx=1, pady=0, sticky="we")
            ...
            for idx in range(0, len(self.psub_fields) + 1, 1):
                for psub in self.psub_fields:
                    ...
                    psubs["t_value"+psub+str(idx)] = tk.Entry(self.root, textvariable=psubs["t"+psub+str(idx)], state="readonly", justify="center", width=8)
                    psubs["t_value"+psub+str(idx)].grid(row=self.data_rows, column=column,
        ...
        for mac1 in d:
            for mac2 in d[mac1]:
                ...
                if not macs in self.data_boxs:
                    ...
                    self.data_boxs[macs + "v"] = tk.Entry(self.root, textvariable=self.data_boxs[macs], state="readonly", width=32)
                    self.data_boxs[macs + "v"].grid(row=self.data_rows, column=3, padx=1, pady=5, sticky="ew")
                ...
                for pkt_filed in self.ptitle_fields:
                    ...
                    if rssiv in self.data_boxs:
                        ...
                        self.data_boxs[rssiv + "v"] = tk.Entry(self.root, textvariable=self.data_boxs[rssiv], state="readonly", width=8, justify="center")
                    ...
                    if cntv in self.data_boxs:
                        ...
                        self.data_boxs[cntv + "v"] = tk.Entry(self.root, textvariable=self.data_boxs[cntv], state="readonly", width=8, justify="center")
                    ...
                    if retryv in self.data_boxs:
                        ...
                        self.data_boxs[retryv + "v"] = tk.Entry(self.root, textvariable=self.data_boxs[retryv], state="readonly", width=8, justify="center")
