...
def rshark_from_conf(file):
    with open(file, "r") as conf_file:
        while True:
            ...
            l = line.split("!")
            ## user, pass, port, ip, rpath, use tunnel, type
            lhost = {}
            if int(l[5]) > 0:
                lhost["user"] = l[0]
                lhost["password"] = l[1]
                ...
                lhost["type"] = l[6]
                lhost["channel"] = l[7]
                lhost["interface"] = l[8].strip(",").split(",")
                lhost["timeout"] = 10
                lhost["dst"] = "local://" + os.getcwd() + "/remote/" + lhost["ip"]
                conf_hosts.append(lhost)
    pass

def rshark_lookup_hosts(ip, ifraise):
...
class Rshark():
    def __init__(self, rtype, rip, rport, ruser, rpasswd, lstore, intf, channel, macs, timeout):
        ...
        if store_types[store_tmp[0]]:
            if not store_tmp[1] and store_types[store_tmp[0]]["need_path"]:
                raise
    ...
    def rshark_store_local(self, arg, inputd):
        ...
        dst_file = path.rstrip("/") + "/" +str(time.localtime().tm_year) + "_" +\
...
if __name__ == "__main__":
    ...
    parse.add_argument("-f", "--file", help="if conf is from file", required=False, action="store_true")
    #args = parse.parse_args(["-f", "--file"])

    parse.add_argument("--conf", help="path to the config file", required=True, type=str)
    ...
    parse.add_argument("--dst", help="where to store the sniffer log, show start with: local://yourpath OR wireshark://.", required=False, type=str)
